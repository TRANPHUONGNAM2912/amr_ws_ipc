...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode")
{
  ...
  else
  {
    ...
    // Identity quaternion (w=1) is required; w=0 makes an invalid rotation.
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    last_scan = *new_scan;
    for (auto &r : last_scan.ranges) {
      // Many lidars encode "no return" as > range_max (e.g. 61.0 when max is 60.0).
      // RF2O treats finite values as valid; convert out-of-range samples to 0 (invalid).
      if (!std::isfinite(r) || r <= last_scan.range_min || r > last_scan.range_max) {
        r = 0.f;
      }
    }
    ...
    {
      // Initialize only after TF(base_frame_id <- laser_frame) is available.
      // If TF is not ready yet during startup, keep waiting for next scan.
